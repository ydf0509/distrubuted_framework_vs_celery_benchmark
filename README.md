...
1、主要是很多情况下函数里面是io任务，如果你for循环单线程运行，那么celery肯定是暴击for循环单线程运行函数的。
   如果消费函数是print 一下消息而已，那你for循环运行这个函数每秒钟达到5到10万次不是梦。处理这种函数任务，此时celery不仅不会变快，反而会大幅拉后腿。
...
4、此篇性能测试，是方便用户对celery有一个大致的了解，不然很容易陷入以为只需要给运维总监提需求购买一个双核4g的阿里云，就能达到每秒执行百万次任务，到时候生产达不到这个性能指标目的，
    那自己就要背锅了，因为是你让运维总监只采购一台双核4g阿里云的。
```